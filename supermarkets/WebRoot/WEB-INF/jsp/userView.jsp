...
<div class="right">
	...
	<div class="providerView">
		...
		<p>
			<strong>用户角色：</strong><span>${user.userRole}</span>
		</p>
		<p>
			<strong>个人证件照：</strong><span> <c:choose>
					...
					<c:otherwise>
						<img src="/supermarkets/statics/uploadfiles/${user.idPicPath}" style="width: 100px;height: 100px" />
					</c:otherwise>
				</c:choose> </span>
		</p>
		<p>
			<strong>工作证照片：</strong><span> <c:choose>
					<c:when
						test="${user.workPicPath == null || user.workPicPath == ''}">
            	 	暂无
            	</c:when>
					<c:otherwise>
						<img src="/supermarkets/statics/uploadfiles/${user.workPicPath}" style="width: 100px;height: 100px" />
					</c:otherwise>
				</c:choose> </span>
		</p>
